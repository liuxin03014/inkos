...
${outputFormat}`;
  ...
  const reducedControlBlock = chapterIntent && contextPackage && ruleStack
    ? buildReducedControlBlock(chapterIntent, contextPackage, ruleStack)
    : "";
...
${chapterContent}`;
  ...
  const output = parseRepairOutput(response.content, gp, mode, chapterContent);
